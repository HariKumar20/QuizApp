...
function ScoreCalculate({navigation , route})
{
    ...
    for(let i=0 ;i<objectData.questionsData.length ;i++)
    {
        if(route.params.answers[objectData.questionsData[i].question] == objectData.questionsData[i].correct_option)
        {
            score++;
        }
    }
    ...
    return(
        <View style={styles.container}>
            ...
            <ScrollView>
                {
                    objectData.questionsData.map((questionIndex , index)=>
                    <View style={styles.questionView} key={index}>
                        ...
                        <Text style={(questionIndex.correct_option == route.params.answers[questionIndex.question]) ? styles.showresult : styles.hideresult}>✅</Text>
                        <Text style={(questionIndex.correct_option != route.params.answers[questionIndex.question]) ? styles.showresult : styles.hideresult}>❌</Text>
                        {
                        questionIndex.options.map((optionIndexing ,indexpos) =>
                        <Text style={(optionIndexing.option_id == questionIndex.correct_option)?styles.questionresponse : styles.hideresult} key={indexpos}>The Answer is {optionIndexing.option}</Text>)
                        }
